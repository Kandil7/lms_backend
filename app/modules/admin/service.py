...
class AdminService:
    ...
    def configure_admin_security(
        ...
        security_payload = security_config.model_dump()
        ...
        admin.security_health_score = self._calculate_security_score(security_payload)

        # Update other security settings
        if hasattr(admin, "security_level"):
            admin.security_level = (
                "maximum" if security_config.mfa_method == "hardware" else "enhanced"
            )
