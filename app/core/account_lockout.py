...
class AccountLockoutManager:
    def __init__(self):
        ...
        self.lockout_duration = settings.ACCOUNT_LOCKOUT_DURATION_SECONDS
    ...
    def increment_failed_attempts(self, email: str, ip_address: str) -> int:
        ...
        # Set expiration for both (configurable duration)
        self.cache.expire(user_key, self.lockout_duration)
        self.cache.expire(ip_key, self.lockout_duration)
    ...
    def is_account_locked(self, email: str, ip_address: str) -> bool:
        ...
        return (
            user_attempts >= settings.MAX_FAILED_LOGIN_ATTEMPTS
            or ip_attempts >= settings.MAX_FAILED_LOGIN_ATTEMPTS
        )
