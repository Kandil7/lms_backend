@echo off
setlocal EnableExtensions EnableDelayedExpansion

set "NO_BUILD=0"
set "SKIP_MIGRATE=0"
set "SKIP_SEED=0"
set "FOLLOW_LOGS=0"
set "SCRIPT_DIR=%~dp0"

:parse_args
if "%~1"=="" goto args_done
if /I "%~1"=="-NoBuild" set "NO_BUILD=1" & shift & goto parse_args
if /I "%~1"=="--no-build" set "NO_BUILD=1" & shift & goto parse_args
if /I "%~1"=="-SkipMigrate" set "SKIP_MIGRATE=1" & shift & goto parse_args
if /I "%~1"=="--skip-migrate" set "SKIP_MIGRATE=1" & shift & goto parse_args
if /I "%~1"=="-SkipSeed" set "SKIP_SEED=1" & shift & goto parse_args
if /I "%~1"=="--skip-seed" set "SKIP_SEED=1" & shift & goto parse_args
if /I "%~1"=="-FollowLogs" set "FOLLOW_LOGS=1" & shift & goto parse_args
if /I "%~1"=="--follow-logs" set "FOLLOW_LOGS=1" & shift & goto parse_args
if /I "%~1"=="-h" goto show_help
if /I "%~1"=="--help" goto show_help
echo Unknown option: %~1
goto show_help

:args_done
for %%I in ("%SCRIPT_DIR%..") do set "PROJECT_ROOT=%%~fI"
if not defined PROJECT_ROOT (
    echo ERROR: Failed to resolve project root.
    exit /b 1
)

pushd "%PROJECT_ROOT%" 2>nul
if errorlevel 1 (
    echo ERROR: Failed to switch to project root.
    exit /b 1
)

where docker >nul 2>&1
if errorlevel 1 (
    echo ERROR: docker command not found in PATH.
    popd
    exit /b 1
)

echo.
echo ==^> Preparing environment file
if not exist .env (
    if not exist .env.example (
        echo ERROR: Missing both .env and .env.example
        popd
        exit /b 1
    )
    copy /Y .env.example .env >nul
    echo Created .env from .env.example
) else (
    echo .env already exists
)

set "COMPOSE_FILE=%PROJECT_ROOT%\docker-compose.yml"

echo.
echo ==^> Starting containers
if "%NO_BUILD%"=="1" (
    docker compose -f "%COMPOSE_FILE%" up -d
) else (
    docker compose -f "%COMPOSE_FILE%" up -d --build
)
if errorlevel 1 (
    popd
    exit /b 1
)

if "%SKIP_MIGRATE%"=="0" (
    echo.
    echo ==^> Applying migrations
    docker compose -f "%COMPOSE_FILE%" exec -T api alembic upgrade head
    if errorlevel 1 (
        popd
        exit /b 1
    )
)

if "%SKIP_SEED%"=="0" (
    echo.
    echo ==^> Seeding demo data
    docker compose -f "%COMPOSE_FILE%" exec -T api python scripts/seed_demo_data.py --reset-passwords
    if errorlevel 1 (
        popd
        exit /b 1
    )
)

echo.
echo ==^> Waiting for API health endpoint
set "HEALTH_OK=0"
for /L %%I in (1,1,30) do (
    curl.exe -fsS "http://localhost:8000/api/v1/health" >nul 2>&1
    if !errorlevel! EQU 0 (
        set "HEALTH_OK=1"
        goto health_done
    )
    timeout /t 2 /nobreak >nul
)

:health_done
if "%HEALTH_OK%"=="1" (
    echo API is healthy at http://localhost:8000/api/v1/health
) else (
    echo WARNING: API health check did not become ready in time.
)

echo.
echo ==^> Demo environment is ready
echo Docs:   http://localhost:8000/docs
echo Health: http://localhost:8000/api/v1/health
echo.
echo Demo credentials:
echo - admin.demo@example.com / AdminPass123
echo - instructor.demo@example.com / InstructorPass123
echo - student.demo@example.com / StudentPass123

if "%FOLLOW_LOGS%"=="1" (
    echo.
    echo ==^> Following logs (Ctrl+C to stop)
    docker compose -f "%COMPOSE_FILE%" logs -f api celery-worker celery-beat
)

popd
exit /b 0

:show_help
echo.
echo Usage:
echo   scripts\run_demo.bat [options]
echo.
echo Options:
echo   -NoBuild ^| --no-build
echo   -SkipMigrate ^| --skip-migrate
echo   -SkipSeed ^| --skip-seed
echo   -FollowLogs ^| --follow-logs
echo   -h ^| --help
echo.
exit /b 1
