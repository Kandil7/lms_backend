...
"@ | Set-Content -Path $envPath
    ...
    $remoteScriptLines = @(
        "#!/usr/bin/env bash"
        "set -euo pipefail"
        "sed -i 's/\r$//' /tmp/lms_backend_demo_deploy.env"
        "set -a"
        ...
        "cd ""`$`{APP_DIR`}"""
        "sed -i 's/\r$//' scripts/platform/linux/deploy_azure_demo_vm.sh"
        "chmod +x scripts/platform/linux/deploy_azure_demo_vm.sh"
        "DEPLOY_MODE=vm bash ./scripts/platform/linux/deploy_azure_demo_vm.sh"
