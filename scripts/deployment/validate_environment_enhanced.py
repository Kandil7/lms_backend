#!/usr/bin/env python3
"""
Enhanced environment validation for production deployment.

This script extends the original validate_environment.py with additional security checks
for hardcoded secrets and improves validation coverage.
"""
...
class EnvironmentValidator:
    def __init__(self, is_example: bool = False) -> None:
    ...
    def validate_required_flags(self) -> None:
        # Production-specific requirements - skip for example files
    ...
    def validate_secret_key(self) -> None:
        ...
        if lower in INSECURE_PLACEHOLDERS or any(placeholder in lower for placeholder in INSECURE_PLACEHOLDERS):
    ...
    def validate_hardcoded_secrets(self) -> None:
        """Additional validation for hardcoded secrets in environment variables"""
        # Skip validation for example files that are clearly templates
        if self.is_example:
            # For example files, only warn about truly dangerous hardcoded values
            for key, value in os.environ.items():
                ...
                # Check for truly dangerous hardcoded values (not just placeholders)
                dangerous_values = ["admin", "password", "123456", "qwerty", "root"]
                for dangerous in dangerous_values:
                    if dangerous in lower_value and len(value) < 8:
                        self._error(f"Environment variable {key} contains dangerous hardcoded value: '{value}'")
                ...
                for pattern in placeholder_patterns:
                    if pattern in lower_value:
                        self._warn(f"Environment variable {key} contains placeholder '{pattern}' - replace before production use")
        else:
            # Full validation for production files
            for key, value in os.environ.items():
                if not value:
                    continue
                ...
                # Check for insecure placeholders
                for placeholder in INSECURE_PLACEHOLDERS:
                    if placeholder in lower_value:
                        self._error(f"Environment variable {key} contains insecure placeholder: '{placeholder}'")
                
                # Check for weak passwords and default credentials
                if any(keyword in lower_value for keyword in ["admin", "password", "pass", "secret", "123", "qwerty"]):
                    if len(value) < 8 or value.isalnum() and not any(c.isalpha() for c in value) or value.isalpha():
                        self._warn(f"Environment variable {key} may contain weak password/credential: '{value[:20]}...'")

    def run(self) -> tuple[bool, list[str], list[str]]:
        LOGGER.info("Starting enhanced environment validation")
    ...
    def print_report(self) -> None:
        print("\n" + "=" * 68)
        print("ENHANCED ENVIRONMENT VALIDATION REPORT")
        print("=" * 68)
        if self.errors:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate production environment settings with enhanced security checks")
    parser.add_argument(
        "--env-file",
        type=Path,
        default=None,
    ...
    parser.add_argument(
        "--example",
        action="store_true",
        help="Validate example files (skip production-specific requirements)",
    )
...
def main() -> int:
    try:
        ...
        validator = EnvironmentValidator(is_example=args.example)
